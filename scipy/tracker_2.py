...
def load_detector():
    if not Path(YUNET_MODEL_PATH).exists():
        print(f"Error: Model YuNet tidak ditemukan di '{YUNET_MODEL_PATH}'")
        return None
    try:
        ...
        print(f"Error saat memuat model YuNet: {e}")
        return None
...
def detect_faces_yunet(detector, frame):
    ...
    if faces is not None:
        for face in faces:
            ...
            results.append((y1, x1, y2, x2)) # Format: top, left, bottom, right
    return results
...
def extract_embedding(embedder, face_img):
    ...
    embedder.set_tensor(emb_input[0]['index'], input_tensor)
    embedder.invoke()
...
def connect_db():
    try:
        return mariadb.connect(user="raspberry", password="pisang123", host="192.168.100.240", port=3306, database="attedance", connect_timeout=5)
...
def record_attendance(user_id):
    ...
    try:
        cursor = conn.cursor()
        now = datetime.now()
        tgl, tipe = now.date(), "masuk" if now.hour < 12 else "pulang"
...
# === WORKER THREAD (DENGAN LOGIKA TRACKING) ===
class ProcessingThread(QThread):
    ...
    def run(self):
        ...
        if detector is None:
            self.status_updated.emit("CRITICAL: Gagal memuat model detektor YuNet!")
            return
        ...
        while self.running and cap.isOpened():
            ...
            if self.frame_count % DETECTION_INTERVAL == 0:
                ...
                self.update_trackers(frame_for_display, face_locations)
            else:
                self.track_faces(frame_for_display)

            # --- Proses & Gambar Hasil Tracking ---
            ...
            live_name, live_dept, live_score, live_ref_path = "", "", 0.0, None
                        ...
                        try:
                            ...
                            if sims:
                                ...
                                if score >= RECOGNITION_THRESHOLD:
                                    ...
                                    data['name'] = name
                                    live_name, live_dept, live_score, live_ref_path = name, department, score, ref_path
                                    now_time = time.time()
            ...
            self.frame_ready.emit(cv2.cvtColor(frame_for_display, cv2.COLOR_BGR2RGB), live_ref_path, live_name, live_dept, live_score)
            self.frame_count += 1
    ...
    def track_faces(self, frame):
        for face_id, data in list(self.active_trackers.items()):
            ...
            if success:
                ...
                data['box'] = (y, x, y + h, x + w) # Konversi ke top, left, bottom, right
                data['disappeared'] = 0
            else:
                data['disappeared'] += 1
    ...
    def update_trackers(self, frame, face_locations):
        if not face_locations and not self.active_trackers:
            return
        ...
        if len(tracked_centroids) > 0:
            ...
            for col in unused_cols:
                self.register_new_tracker(frame, face_locations[col])
        else:
            for loc in face_locations:
                self.register_new_tracker(frame, loc)
    
    def register_new_tracker(self, frame, box):
        top, left, bottom, right = box
        tracker_box = (left, top, right - left, bottom - top) # Format (x, y, w, h)
        try:
            tracker = cv2.TrackerKCF_create()
            tracker.init(frame, tracker_box)
        except Exception:
            try: # Fallback untuk beberapa versi OpenCV
                tracker = cv2.legacy.TrackerKCF_create()
                tracker.init(frame, tracker_box)
            except Exception as e:
                print(f"Gagal menginisialisasi tracker KCF: {e}"); return
...
class AttendanceApp(QWidget):
    ...
    def update_live_view(self, frame_rgb, ref_path, name, department, score):
        ...
        if name and ref_path and Path(ref_path).exists():
            ref_pixmap = QPixmap(ref_path); self.live_ref_image_label.setPixmap(ref_pixmap.scaled(150, 150, Qt.KeepAspectRatio, Qt.SmoothTransformation))
